...
public interface EntryRepository extends JpaRepository<Entry, Long> {
    ...
    @Query("select e from Entry e " +
            "where ((:filter is null) or (:filter = '') or (lower(e.title) like lower(concat('%', :filter, '%')))) " +
            "and (:statuses is null or e.status in :statuses) " +
            "order by e.created desc")
    List<Entry> findAll(@Param("filter") String filter, @Param("statuses") Collection<Status> statuses);
